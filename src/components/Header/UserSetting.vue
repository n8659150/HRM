<template>
  <div class="user-setting">
    ...
    <div class="tag-list">
      <span style="margin-right: 0.5rem" v-for="(tag, $key) in tags" :key="$key">
    ...
    <div class="tag-list">
      <span style="margin-right: 0.5rem" v-for="(highlight, $key) in highlights" :key="$key">
        <b-badge variant="warning" size="default">
          {{ highlight.content }}
          <span
            class="inline-block"
            style="margin-left: 0.5rem;"
            v-on:click="deleteHighlight(highlight.id)"
          >
            <i class="fa fa-remove" aria-hidden="true"></i>
          </span>
        </b-badge>
